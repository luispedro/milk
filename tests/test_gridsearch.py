...
from milk.supervised.gridsearch import gridmaximise, _allassignments
import numpy as np
...
def test_gridmaximise():
    ...
    x = gridmaximise(milk.supervised.svm_simple(kernel=np.dot, C=2.), features[::2], labels[::2] == 0, {'C' : (0.5,) })
    cval, = x
    assert cval == ('C', .5)
